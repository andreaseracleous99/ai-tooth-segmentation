# step5_train_binary_classifier.py
...
def seed_everything(seed: int = 42):
    ...
    torch.backends.cudnn.benchmark = True       # faster if image sizes constant
...
def main():
    ...
    # If you train on CPU, increase batch_size only if you have enough RAM.
    BATCH_SIZE = 64
    NUM_WORKERS = 0  # Windows: keep 0 to avoid multiprocessing issues

    # Training schedule (recommended)
    WARMUP_EPOCHS = 1          # train only fc
    FINETUNE_EPOCHS = 6        # unfreeze layer4 + fc
    LR_WARMUP = 1e-3
    LR_FINETUNE = 1e-4
    ...
    # -------------------------
    # Use more CPU threads (helps on CPU)
    # -------------------------
    ...
    # Transforms
    # -------------------------
    # For radiographs, contrast/brightness/equalize is more useful than heavy rotation.
    train_tf = transforms.Compose([
        ...
        # Small rotation/flip is OK (patches might be mirrored)
    ...
    # IMPORTANT:
    # Create a *separate* ImageFolder for val so it uses val_tf (not train_tf)
    ...
    # -------------------------
    # Compute class weights (from TRAIN indices only)
    # -------------------------
    # ImageFolder stores labels in .targets aligned with .samples
    ...
    def train_epochs(num_epochs: int, lr: float, stage: str, best_acc: float):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                best_path = OUT_DIR / "tooth_vs_nontooth_resnet18_best.pth"
                torch.save({
    ...
    # final save
    final_path = OUT_DIR / "tooth_vs_nontooth_resnet18_final.pth"
    torch.save({
        "model_state": model.state_dict(),
        "classes": classes,
        "img_size": IMG_SIZE,
